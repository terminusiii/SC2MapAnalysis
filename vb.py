...
def bumpminor():
    ...
    minor = minor.replace(".", "")
    click.echo("Current Version: " + click.style(old_version, fg="green"))
    click.echo("Minor Found: " + click.style(minor, fg="green"))
    bump = str(int(minor) + 1)
    click.echo("Bumping to : " + click.style(bump, fg="blue"))
    new_version = str(old_version).replace(minor, bump)
    click.echo("Updated Version: " + click.style(new_version, fg="red"))
    b_minor(new_version)
...
def gh(days):
    click.echo(
        click.style("Showing last ", fg="blue")
        + click.style(days, fg="green")
        + click.style(" days summary", fg="blue")
    )
    subprocess.check_call("git fetch", shell=True)
    subprocess.check_call(
        f"git log --oneline --decorate --graph --all -{days}", shell=True
    )
